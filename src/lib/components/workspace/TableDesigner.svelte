<script lang="ts">
  ...
  async function seed() {
    ...
    if (cid && st.table) {
      try {
        ...
        cols = existing.map((c) => ({ name: c.name, type: c.data_type, len: '', pk: c.is_pk, nullable: c.nullable, dflt: c.default ?? '', existing: true }))
      } catch {
        cols = []
      }
      try {
      ...
      try {
        ...
        triggers = trg.filter((t) => t.table === st.table).map((t) => ({ name: t.name, timing: '', event: t.event, body: '', existing: true }))
      } catch {
        /* keep */
      }
    }
  ...
  const delCol = (i: number) => (cols = cols.filter((_, idx) => idx !== i))
  const addIndex = () => (indexes = [...indexes, { name: '', columns: [], method: '' }])
  const delIndex = (i: number) => (indexes = indexes.filter((_, idx) => idx !== i))
  const addFk = () => (fks = [...fks, { name: '', columns: [], refTable: '', refColumns: [], onDelete: '', onUpdate: '' }])
  const delFk = (i: number) => (fks = fks.filter((_, idx) => idx !== i))
  const addUnique = () => (uniques = [...uniques, { name: '', columns: [] }])
  const delUnique = (i: number) => (uniques = uniques.filter((_, idx) => idx !== i))
  const addCheck = () => (checks = [...checks, { name: '', expression: '' }])
  const delCheck = (i: number) => (checks = checks.filter((_, idx) => idx !== i))
  const addTrigger = () => (triggers = [...triggers, { name: '', timing: 'BEFORE', event: 'INSERT', body: '' }])
  const delTrigger = (i: number) => (triggers = triggers.filter((_, idx) => idx !== i))
    ...
    <div style="flex:1;overflow:auto;min-height:0">
        ...
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each cols as col, i (i)}
              <tr style={col.existing ? 'opacity:0.75' : ''}>
        ...
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each indexes as ix, i (i)}
              <tr style={ix.existing ? 'opacity:0.7' : ''}>
                ...
                <td style="border-bottom:var(--px-1) solid var(--border);text-align:center">{#if !ix.existing}<span onclick={() => delIndex(i)} onkeydown={(e) => e.key === 'Enter' && delIndex(i)} role="button" tabindex="0" style="cursor:pointer;color:var(--muted);font-size:var(--px-14)">×</span>{/if}</td>
              </tr>
            {/each}
          </tbody>
        </table>
      ...
      {:else if activeTab === 'foreign-keys'}
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each fks as fk, i (i)}
              <tr style={fk.existing ? 'opacity:0.7' : ''}>
                ...
                <td style="border-bottom:var(--px-1) solid var(--border);text-align:center">{#if !fk.existing}<span onclick={() => delFk(i)} onkeydown={(e) => e.key === 'Enter' && delFk(i)} role="button" tabindex="0" style="cursor:pointer;color:var(--muted);font-size:var(--px-14)">×</span>{/if}</td>
              </tr>
            {/each}
          </tbody>
        </table>
        ...
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each uniques as u, i (i)}
              <tr style={u.existing ? 'opacity:0.7' : ''}>
                ...
                <td style="border-bottom:var(--px-1) solid var(--border);text-align:center">{#if !u.existing}<span onclick={() => delUnique(i)} onkeydown={(e) => e.key === 'Enter' && delUnique(i)} role="button" tabindex="0" style="cursor:pointer;color:var(--muted);font-size:var(--px-14)">×</span>{/if}</td>
              </tr>
            {/each}
          </tbody>
        </table>
        ...
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each checks as c, i (i)}
              <tr style={c.existing ? 'opacity:0.7' : ''}>
                ...
                <td style="border-bottom:var(--px-1) solid var(--border);text-align:center">{#if !c.existing}<span onclick={() => delCheck(i)} onkeydown={(e) => e.key === 'Enter' && delCheck(i)} role="button" tabindex="0" style="cursor:pointer;color:var(--muted);font-size:var(--px-14)">×</span>{/if}</td>
              </tr>
            {/each}
          </tbody>
        </table>
        ...
        <table style="border-collapse:collapse;width:100%;font-size:var(--px-12_5)">
          ...
          <tbody>
            {#each triggers as tr, i (i)}
              <tr style={tr.existing ? 'opacity:0.7' : ''}>
                ...
                <td style="border-bottom:var(--px-1) solid var(--border);text-align:center">{#if !tr.existing}<span onclick={() => delTrigger(i)} onkeydown={(e) => e.key === 'Enter' && delTrigger(i)} role="button" tabindex="0" style="cursor:pointer;color:var(--muted);font-size:var(--px-14)">×</span>{/if}</td>
              </tr>
            {/each}
          </tbody>
        </table>
