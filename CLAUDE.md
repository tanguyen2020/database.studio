...
- **T15 — Generate Scripts whole schema/DB — ⏳ NEXT** (known gap #3: structure-only/data-only/both; multi-object; dependency order — child after parent, FK last, view after base; per-dialect; re-add SQLite Export.sql + Explorer Export/dump. Integration: generate structure for seeded PG schema → run into fresh schema → introspection identical; CH round-trips SHOW CREATE).
- T16..T23 — pending (see SPEC_SUPPLEMENT.md "Implementation priority").
