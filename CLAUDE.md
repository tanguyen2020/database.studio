...
- **T16 — Query Plan per-system — ⏳ NEXT** (MSSQL SHOWPLAN_XML parse; CH EXPLAIN tree + indexes; MariaDB ANALYZE actual; Cassandra TRACING timeline; hotspot rules. Integration per system: EXPLAIN seeded table → normalized root/child node w/ expected op; Cassandra ALLOW FILTERING flagged. Commit per-system as each goes green).
- T17..T23 — pending (see SPEC_SUPPLEMENT.md "Implementation priority").
