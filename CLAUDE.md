...
- **T14 — Table/Query Export Wizard — ⏳ NEXT** (export dialog format/WHERE/columns/limit/filename for table + current result; large-set streaming; unit per-format serialization incl. column subset + WHERE; integration export seeded PG → CSV/SQL re-import match count).
- T15..T23 — pending (see SPEC_SUPPLEMENT.md "Implementation priority").
