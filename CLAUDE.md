...
- **T17 — Index Scanner completeness — ⏳ NEXT** (CH `system.data_skipping_indices` + Cassandra `system_schema.indexes` adapters; anti_pattern flag; missing-index suggestions PG repeated seq-scan / MSSQL DMV. Integration: seed indexes on CH/Cassandra, scan returns them + correct flags).
- T18..T23 — pending (see SPEC_SUPPLEMENT.md "Implementation priority").
