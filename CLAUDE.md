...
- **T21 — Timeout/retry/pooling + transactions + autocomplete + shortcuts — ⏳ NEXT** (connection pool size/idle/acquire + retry/backoff Settings→Connections; explicit BEGIN/COMMIT/ROLLBACK (disabled CH); autocomplete function signatures; bind shortcuts Ctrl+Shift+F, Ctrl+Alt+G/J/R, Ctrl+Shift+C, Ctrl+F. unit pool config + shortcut map; integration concurrent tabs share pool + rollback discards seeded insert).
- T22..T23 — pending (see SPEC_SUPPLEMENT.md "Implementation priority").
