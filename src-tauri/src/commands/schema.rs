...
pub fn definition_query(system: &str, kind: &str, schema: &str, name: &str) -> Option<String> {
    ...
    let q = match (system, kind) {
        ...
        // Oracle: DBMS_METADATA.GET_DDL → the full CLOB (the `oracle` crate reads
        // LOBs completely, so no length cap).
        ("oracle", kind) => {
            let t = match kind {
                "view" => "VIEW",
                "trigger" => "TRIGGER",
            ...
            format!("SELECT DBMS_METADATA.GET_DDL('{t}', '{n}', '{s}') AS d FROM DUAL")
        }
...
pub fn index_definition_query(system: &str, schema: &str, table: &str, name: &str) -> Option<String> {
    ...
    let q = match system {
        ...
        // Oracle: GET_DDL('INDEX', …) → the full CLOB (no length cap on crate A).
        "oracle" => format!("SELECT DBMS_METADATA.GET_DDL('INDEX', '{n}', '{s}') AS d FROM DUAL"),
        _ => return None,
    };
    Some(q)
}
