...
+ ConnectionForm nút: **Test → "Test connection"**, **Save → "Connect"** (label rõ hành động; state: "Testing…"/"Connecting…").
Visual `connection-form.spec` (nút chạy, không còn Group, Cancel đóng form). Gates: check 0/0, vitest 179, playwright 35.

**A7 — i18n sweep: toàn bộ user-facing text = English (Fixed).**
Rà TẤT CẢ component + store + demo cho chuỗi tiếng Việt hiển thị cho người dùng (label/placeholder/toast/error/empty-state/generated-SQL comment), chuyển hết sang English. Phạm vi đã sửa ngoài khu Connections (A6): App.svelte (welcome/onboarding/error-boundary/starting), stores `connections`/`results` (toast load/save/delete/duplicate/reconnect/cancel/"Query cancelled"/"Ran N statement(s)"), RedisPubSub/KafkaProducer/KafkaConsumer (stream + subscribe/publish/consume + empty-state), SavedQueriesTab/HistoryTab/CommandPalette/TabBar/ResultJsonView/ResultGrid/ImportDialog/ExportDialog/ClickHouseTtlDialog/TitleBar, `import/plan.ts` (JSON-array error), `compare/diff.ts` (migration header + routine-diff comment), `demo.ts` (CQL JOIN error, TTL human text, index-suggestion reason, unmocked-command error). **Chỉ chừa comment code + test descriptions (developer-facing) + dữ liệu mẫu (vd tên 'Bình') + ký tự `×`** — không phải UI text. Specs cập nhật theo text mới: `connection-form` (Test connection), `explorer-depth` (placeholder "Filter tree"), `backup` ("Tool:"), `schema-compare` ("Migration: sync TARGET"). Guard: `rg` dấu tiếng Việt trong `toasts.*`/`throw new Error`/`.show(` → 0 match. Gates: check 0/0, vitest 179, playwright 35.
