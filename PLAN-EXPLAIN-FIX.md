...
### EXP-P3.3 — Actual thật cho MySQL/MSSQL (stretch) — ✅ DONE
> Branch `feat/explain-actual`, commit `EXP-P3.3`. **MySQL**: `build_explain` mysql+actual → `EXPLAIN ANALYZE` (TREE text, 8.0.18+); parser MỚI `parse_mysql_tree` (stack theo indent trước `->`; nhãn → op qua `mysql_tree_op`; metric từ 2 nhóm `(cost=…)`/`(actual …)`; actual_rows = rows×loops, actual_time = B×loops; cost cumulative → `assign_cost_pct(true)`). **MSSQL**: `explain_mssql(actual)` → `SET STATISTICS XML ON` (thực thi query, plan kèm RunTimeInformation) → quét cell chứa `<ShowPlanXML` (plan lẫn trong result set flatten); `parse_mssql_xml(xml, actual)` + `build_mssql_node(…, actual)` đọc `<RunTimeCountersPerThread>` (sum ActualRows, max ActualElapsedms) → mode=actual. **Capability** mysql/mssql → `actual_kind=Analyze` (toggle Actual hiện). DML+actual vẫn bị chặn bởi P0.1 (MySQL/MSSQL không postgres → block). Frontend: demo capability mysql/mssql → analyze. Tests: unit `mysql_explain_analyze_tree` + `mssql_statistics_xml_actual` + capability/build_explain cập nhật (Rust lib plan 25); integration `xv_t1_mysql_scan_index_and_actual` (EXPLAIN ANALYZE → mode actual + actual_rows) + `xv_t2_mssql_scan_index_and_actual` (STATISTICS XML → RunTimeInformation → actual_rows) EXIT=0; e2e query-plan: MySQL hiện Actual toggle + ClickHouse ẩn (thay test cũ). **Lưu ý**: MySQL <8.0.18 EXPLAIN ANALYZE lỗi → surface typed error (không version-gate; hiếm). Gates: check 0/0, vitest 510, e2e query-plan 5/5.
...
### FIX — Run đóng Query Plan sub-tab + Export menu đóng khi click ngoài — ✅ DONE
> Commit `EXP-FIX-run-closes-plan`. (1) User: "Run phải focus tab result + đóng Query Plan; chỉ Explain mới mở plan." → `run()`/`runAtCursor()`/SQLite PRAGMA run gọi `results.clearExplain(tab.id)` trước `results.run` → sub-tab "Query Plan" biến mất, ResultPanel effect set `planActive=false` → hiện result; Explain mở lại. (2) User: Export ▾ (ResultPanel) đóng khi click ra ngoài → thêm backdrop `position:fixed;inset:0;z-index:19` (dưới dropdown z-20) onclick đóng (dropdown = đóng khi click ngoài, đúng rule AUDIT-13c). e2e: query-plan +1 (Run→plan count 0, Explain→count 1); result-pager +assert Export menu click-outside đóng. Gates: check 0/0, e2e query-plan 4/4 + result-pager 1/1.
